...
test("Inertia navigation and Rails form validation complete without a full-page API layer", async ({ page }) => {
  await page.goto("/")
  ...
  await expect(page.getByRole("alert")).toContainText("Rails received “Hello from Playwright” through Inertia.")

  const inertiaRequest = page.waitForRequest((request) => request.url().endsWith("/states/empty"))
...
for (const viewport of [
  ...
  test(`responsive shell at ${viewport.name} size`, async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "A boring foundation for useful Rails applications." })).toBeVisible()
...
test("dark mode and representative states remain usable", async ({ page }) => {
  ...
  await expect(page.getByText("All foundation checks are healthy")).toBeVisible()
  ...
  const applicationError = page.waitForResponse((response) => response.url().endsWith("/500"))
  ...
  await expect(page.getByRole("heading", { name: "The application could not complete that request" })).toBeVisible()
})
